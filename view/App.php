				...
				if($get_last_slash !== '/') {
					...
					if(count($_url) <= 1) {
						if(!empty($_url[0])) {
							...
							$path = '../' . $this->global['files_path']['view'] . '/' . $this->global['routes']['default']['path'] . '/index.php';

							include_once '../' . $this->global['files_path']['controller'] . '/' . $this->global['routes']['default']['path'] . '/' . ucfirst($this->global['routes']['default']['path']) . 'Controller.php';
							$path_controller = ucfirst($this->global['routes']['default']['path']) . 'Controller';
