...
class LoginController extends REJController {
	...
	public function userLoginAction() {
		...
		if(!empty($username) && !empty($password)) {
            ...
            $get_user = $db->execQuery("SELECT user_id, first_name, middle_name, last_name, username, password, privilege FROM " . $this->table_prefix . "users WHERE username=:user",array(':user'=>$username),"rows");
            ...
            if(count($get_user) > 0) {
                if(password_verify($password, $get_user[0]['password'])) {
                    ...
                    foreach($get_user as $user_info) {
                        ...
                        $pages_access = $db->execQuery("SELECT 
                                                    acs.pa_id page_id, 
                                                    p.url url,
                                                    p.name name,
                                                    p.icon icon
                                                FROM 
                                                    " . $this->table_prefix . "page_access AS acs 
                                                INNER JOIN 
                                                    " . $this->table_prefix . "pages AS p 
                                                ON acs.pa_id=p.page_id 
                                                WHERE acs.privilege=:priv AND p.parent=:parent GROUP BY p.page_id ORDER BY p.position ASC",array(':priv'=>$user_info['privilege'],':parent'=>0),"rows");
